...
class BlockBasedTable : public TableReader {
 ...
 private:
  ...
  template <typename TBlocklike>
  Status MaybeReadBlockAndLoadToCache_aio(
      FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
      const BlockHandle& handle, const UncompressionDict& uncompression_dict,
      CachableEntry<TBlocklike>* block_entry, BlockType block_type,
  ...
  Status RetrieveBlock_post_aio(
      ...
      const BlockHandle& handle, const UncompressionDict& uncompression_dict,
      CachableEntry<TBlocklike>* block_entry, BlockType block_type,
      GetContext* get_context, bool use_cache, struct iocb* aiocbList_f, AlignedBuffer* buff) const;
...
struct BlockBasedTable::Rep {
  Rep(const ImmutableCFOptions& _ioptions, const EnvOptions& _env_options,
	...
	filter_policy(skip_filters ? nullptr : _table_opt.filter_policy.get()),
        //filter_policy((skip_filters || _level > 3) ? nullptr : _table_opt.filter_policy.get()),
